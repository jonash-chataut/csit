...
// Infix to Postfix conversion with step printing
void infixToPostfix(char infix[], char postfix[])
{
    ...
    for (i = 0; infix[i] != '\0'; i++)
    {
        ...
        printf("| %-13c | %-14s | %-14s |\n", item, (top == -1 ? " " : opStack), operand_Stack);
    }
    ...
    while (top != -1)
    {
        ...
        // Display after popping remaining
        char opStack[MAX] = "", operand_Stack[MAX] = "";
        strncpy(opStack, stack, top + 1);
        strcpy(operand_Stack, postfix);

        printf("| %-13s | %-14s | %-14s |\n", " ", (top == -1 ? " " : opStack), operand_Stack);
    }

    printf("+---------------+----------------+----------------+\n");
}

// Infix to Prefix conversion with steps
